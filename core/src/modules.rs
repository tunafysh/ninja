use crate::shuriken::{kill_process_by_name, kill_process_by_pid};
use chrono::prelude::*;
...
pub async fn make_modules(lua: &Lua) -> Result<(Table, Table, Table, Table, Table, Table, Table, Table)> {
            ...
            Ok(fs::read_dir(path)?
                .map(|e| {
                    e.unwrap()
                        .file_name()
                        .into_string()
                        .unwrap_or_else(|os| format!("<invalid UTF-8: {:?}>", os))
                })
                .collect::<Vec<String>>())
        })?,
    )?;
    ...
    shell_module.set(
        "exec",
        lua.create_function(
            |lua, (command, detached, admin): (String, Option<bool>, Option<bool>)| {
