...
export function ApplicationMenubar({
  ...
  gridView?: "grid" | "list";
  setGridView?: Dispatch<SetStateAction<"grid" | "list">>;
  activeTab?: string;
  activeWindow: string;
}) {
  ...
  return (
    <div
      className={`fixed flex z-999 ${platform === "mac" ? "h-8" : "h-10"} justify-between items-center border-b rounded-t-xl bg-background drag w-full`}
      style={{
              ...
              <DropdownMenuItem
                onClick={() =>
                  activeWindow == "Forge" || activeWindow == "Armory"
                    ? invoke(
                        "toggle_" + activeWindow.toLowerCase() + "_window",
            ...
            <DropdownMenuContent className="w-56">
              {activeWindow == "Ninja" && (
                <>
                  <DropdownMenuRadioGroup
                    value={gridView}
                    onValueChange={setGridView as (value: string) => void}
                  >
