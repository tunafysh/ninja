...
export default function ProviderBox({
  ...
  return (
    <div
      onClick={toggle}
      className={
        `w-24 aspect-square flex flex-col p-1 justify-evenly items-center cursor-pointer select-none bg-muted outline-foreground ${
          enabled ? "outline-2" : "text-gray-500"
        } rounded-md transition-all ` + className
      }
    >
      <div className={enabled ? "grayscale-0" : "grayscale-100"}>{icon}</div>
