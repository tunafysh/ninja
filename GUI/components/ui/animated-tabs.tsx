...
>(({ className, ...props }, ref) => {
  const { hoverStyle, hoveredIndex, activeStyle } =
    React.useContext(TabsContext);

  return (
    <div
      ref={ref}
      className={cn("relative flex space-x-[6px] items-center", className)}
      {...props}
    >
      {/* Hover Highlight */}
      <div
...
>(({ className, index, ...props }, ref) => {
  const { selectedIndex, setSelectedIndex, setHoveredIndex, tabRefs } =
    React.useContext(TabsContext);
...
>(({ className, index, children, ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={cn("mt-2 animate-in fade-in-50 duration-300", className)}
      {...props}
    >
      {children}
    </div>
